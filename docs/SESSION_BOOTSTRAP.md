...
3. **Margin depth (§8).** `MarginTracker` currently enforces a portfolio-aware flat-rate margin model. The `MarginCalculator` Protocol (MM9.4-S1) establishes the architectural seam, and the SPAN parameter data pipeline (MM9.4-S2, `core/risk/span/` — immutable DTOs, parser registry, checksum-verified repository, offline fetch job) provides the deterministic data foundation. The remaining planned enhancement is `SpanMarginCalculator` implementation consuming both (MM9.4-S3/S4).
