...
3. **Margin depth (§8).** `MarginTracker` enforces a portfolio-aware flat-rate margin model by default. When a validated `SpanSnapshot` is available (LIVE F&O with downloaded SPAN parameters), `SpanMarginCalculator` is injected — providing deterministic, scenario-based SPAN margin. The `MarginCalculator` Protocol (MM9.4-S1, `core/risk/margin_calculator.py`) is the substitution seam; the data pipeline (MM9.4-S2, `core/risk/span/`) provides the deterministic snapshot; the calculator (MM9.4-S3) computes SPAN scan + short-option minimum margin; and the composition swap (MM9.4-S4) wires everything at the root. Buying-power integration for the margin gate is the remaining enhancement (planned).
