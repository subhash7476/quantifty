...
- **Tests (counts 2026-09-05; re-verify totals by running the suite):** `tests/runtime/` — 40 test files, incl. the `ast` forbidden-import guard (ADR-002), the seam no-execution-coupling guard, the routing-era ADR-006 guard (the driver is the sole `process_signal` caller), the Runtime Observability Layer suite (`test_telemetry_sink.py` + `test_driver_telemetry.py`), and the ZMQ publishing suites (`test_telemetry_publisher.py` + `test_driver_telemetry_publish.py` + `test_driver_position_publish.py` — metrics H.1 + health H.2 + positions H.3). Full repo: 223 test files under `tests/`. (Prior pinned totals — 214 runtime / 541 repo — are stale; do not quote.)
...
3. **Margin depth (§8) — MET (MM10).** `NseMarginEngine` (`core/risk/nse_margin_engine.py`) is the sole sizing/computation authority (SPAN + credits + ELM); broker RMS is order-acceptance only (ADR-011/012/013). `MarginCalculator` protocol is the substitution seam; `SpanMarginCalculator` is a frozen component.
4. **Broker product model (§9) — RESOLVED (2026-06-15).** `core/brokers/mapping/upstox.py` now emits NRML (`"D"`) for FUTURE/OPTION and CNC (`"D"`) for EQUITY by default. Explicit `ci.product` override path preserved. Adapter `or "I"` fallback unreachable for tradable instruments.
...
6. **Soft strategy residue — RESOLVED per MM11** (see `docs/reports/platform/MM11_REMOVAL_LEDGER.md`): `CaptureEngine`/`metrics_service` removed (MM11.1), `legacy_adapter.save_signal` removed (MM11.1), strategy DDL pruned (MM11.4).
7. **Dead code — RESOLVED per MM11** (same ledger): the seven `core/data/*` legacy twins removed (MM11.2/MM11.3); `AnalyticsProvider` ABC removed (MM11.5).

## Active Priorities

1. **LoopDriver — COMPLETE.** The deterministic runtime is feature-complete (A–H + the Phase G closers: §8.4 `BROKER_ERROR` isolation and the IN-001 kill-switch single source). Remaining execution work is reconciliation depth and ops hardening (`PROJECT_STATE.md` catch-up 2026-09-05), not new driver phases.
2. **Strategy/residue cleanup — COMPLETE per MM11** (removal ledger under `docs/reports/platform/`).
3. **Research venue note:** `core/strategies/` now hosts in-repo sources (`carry_strategy.py`, `knowledge_signal_source.py` behind `GuardedSignalSource`); carry paper/production and NiftyShield PAPER are the live consumers. Constitution §5/ADR-002 tension is governed by ADR-016/020/021 — do not add new sources without the promotion ledger.
...
5. `docs/reports/platform/CAPABILITY_REVIEW.md`, `docs/reports/platform/RUNNER_DEPENDENCY_ANALYSIS.md`, `docs/reports/platform/RUNNER_EXTRACTION_BLUEPRINT.md` — deep context for the loop/observability work.
6. `docs/reports/SALVAGE_REPORT.md` — what was migrated/dropped and why. (Root-level; other reports live under `docs/reports/<carry|ts_basis|sleeves|psb|sfb_f1|index_research|rfa_gate|substrate_csmp|platform|ops_data|strategies>/` since the 2026-09-05 re-org.)

## Current Next Steps

1. Read `docs/PLATFORM_CONSTITUTION.md` + `docs/PROJECT_STATE.md`.
2. The **LoopDriver is feature-complete** (A–H + Phase G closers). Pick up reconciliation depth / ops hardening per `PROJECT_STATE.md` catch-up 2026-09-05 (TDD, suite green-on-merge).
