...
3. **Margin depth (§8).** `MarginTracker` currently enforces a portfolio-aware flat-rate margin model. The `MarginCalculator` Protocol (MM9.4-S1, `core/risk/margin_calculator.py`) establishes the architectural seam. The remaining planned enhancement is `SpanMarginCalculator` implementation (MM9.4-S3/S4).
