...
>
> **⚠️ "Phase H — Telemetry" split (read before recording new work).** In **both** this doc (G — Telemetry integration) and the state docs, "Telemetry" means the **§10 ZMQ `TelemetryPublisher` wire transport**. It is now tracked as **three sub-milestones**: **H.1 Metrics Publishing (§10.2/§10.3) — COMPLETE** and **H.2 Health Publishing (§10.5) — COMPLETE** (both via `core/runtime/telemetry_publisher.py:RuntimeTelemetryPublisher`, driver-driven on the telemetry cadence, best-effort), and **H.3 Position Publishing (§10.4) — REMAINING** (the last §10 gap). Separately, the **"Runtime Observability Layer"** (`core/runtime/metrics.py` in-process counters: `RuntimeMetric`/`TelemetrySink`/`Null`/`InMemoryTelemetrySink`) is the **sole metrics source** that H.1 publishes — a distinct, earlier milestone, *not* the wire transport. (CHANGELOG 2026-06-06; PROJECT_STATE Completed/Planned.)
