...
>
> **⚠️ "Phase H — Telemetry" split (read before recording new work).** In **both** this doc (G — Telemetry integration) and the state docs, "Telemetry" means the **§10 ZMQ `TelemetryPublisher` wire transport**. It is now tracked as **three sub-milestones, all COMPLETE**: **H.1 Metrics Publishing (§10.2/§10.3) — COMPLETE**, **H.2 Health Publishing (§10.5) — COMPLETE**, and **H.3 Position Publishing (§10.4) — COMPLETE** (all via `core/runtime/telemetry_publisher.py:RuntimeTelemetryPublisher` + the driver's `_build_health`/`_build_positions`, driver-driven on the telemetry cadence, best-effort). **§10 telemetry is therefore complete** — `telemetry.{metrics,health,positions}.{node}` are all published. Separately, the **"Runtime Observability Layer"** (`core/runtime/metrics.py` in-process counters: `RuntimeMetric`/`TelemetrySink`/`Null`/`InMemoryTelemetrySink`) is the **sole metrics source** that H.1 publishes — a distinct, earlier milestone, *not* the wire transport. (CHANGELOG 2026-06-06; PROJECT_STATE Completed/Planned.)
