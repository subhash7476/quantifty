...
| 7 | Price basis and CA handling; external CA enumeration with exclusion windows; scoped certification | Price basis ruled (ratio-adjusted as-of-*t*). Store-level certification + in-store CA enumeration COMPLETE (2026-09-15). **G-7 RULED 2026-09-19** (full-span exclusion for non-ratio CAs). **External CA enumeration COMPLETE 2026-09-19** (`PTMS_GANN_P2_CA_ENUMERATION_2026-09-19.md`: NSE CF-CA, 115 G-7 events; P2-a…d ruled). **Still open from the R-13 certificate:** G1/G3/G5 persistence into `n100_audit` (store change, copy-first, operator sign-off) and the 2016-04-19 holiday confirmation | P-2 (external part) | Research |
