...
| 18 | Freeze document committed, SHA-256 recorded, operator-approved | **DRAFT WRITTEN 2026-09-19** (`PTMS_GANN_STAGE1_FREEZE_DOCUMENT_DRAFT_2026-09-19.md`; all RULED — TO TRANSCRIBE rows transcribed; 22 verbatim blocks machine-checked against their sources). The freeze-review items (OPEN-P, OPEN-Q, RR-4 … RR-8, OC-1, OC-2) and the P-3 rulings were **ruled 2026-09-19 and applied in the draft**. **Ready for operator approval.** Item 15 (G-S1) carries the digest, so it follows the freeze commit and precedes any read. Record here, at approval: the document's path in backticks with its `docs/reports/ptms/` prefix, the freeze commit and the SHA-256 of its git blob (64 hex), in the form the screen's guard reads (draft §14, §18). The digest is recorded here, not inside the document | P-5 | Operator |
