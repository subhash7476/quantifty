...
| 3 | **Staleness + fail-fast startup gate implemented** | ⚠ **PARTIAL (gate logic built MM.4; refresh built MM.6; only live wiring remains → MM.7)** | **Gate logic** exists (MM.1–MM.4, `f8b26cd`/`db7a7d4`): `EventType.INSTRUMENT_MASTER_UNAVAILABLE`/`_STALE`, the master check in `_run_startup_gate()` (before reconciliation), `latest_snapshot_date()` + `master_readiness.assess()` + coverage facts. **Refresh job** built (MM.6): validate-before-publish (`validate_and_publish` staging), IST stamping, transactional writes, `run_refresh` + OS-scheduler artifact — tested (365 green); installing the OS task in the target env is the remaining operational step. **Remaining:** the production checker is **injected only in tests**, not wired into a live F&O entry script (**MM.7**). | the `master_readiness` callable is wired into live F&O startup (MM.7). |
