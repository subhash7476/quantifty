...
| 3 | **Staleness + fail-fast startup gate implemented** | ⚠ **PARTIAL (gate logic built MM.4; refresh + live wiring remain → MM.6/MM.7)** | **Gate logic now exists** (MM.1–MM.4, `f8b26cd`/`db7a7d4`): `EventType.INSTRUMENT_MASTER_UNAVAILABLE`/`_STALE`, the master check in `_run_startup_gate()` (before reconciliation), `latest_snapshot_date()` + `master_readiness.assess()` + coverage facts — implemented + tested (352 green). **Remaining:** the OS-scheduled **refresh job** (policy §2 — **MM.6**) does not exist, and the production checker is **injected only in tests**, not wired into a live F&O entry script (**MM.7**). | the refresh job exists (MM.6) **and** the `master_readiness` callable is wired into live F&O startup (MM.7). |
