...
- **Finding 2 — `options_provider` is a second, snapshot-blind reader of the master file → HARD BLOCKER for 4C.7. — RESOLVED 2026-06-08.** `core/data/options_provider.py` read `data/instruments/nse_fo_instruments.duckdb` by raw SQL with no `snapshot_date` filter (`get_weekly_expiry`/strikes/lot_size/expiry-list); once daily snapshots accumulate, `get_lot_size … LIMIT 1` returned an arbitrary snapshot's lot. **Fixed:** all four queries now scope to the latest snapshot (`AND snapshot_date = (SELECT MAX(snapshot_date) FROM instruments)`) — no-op at one snapshot, correct once snapshots accumulate; discovery reads kept in options_provider (resolver is identity-only). +4 TDD tests (`tests/data/test_options_provider_snapshot.py`); suite 308 → 312. This 4C.7 blocker is cleared.
