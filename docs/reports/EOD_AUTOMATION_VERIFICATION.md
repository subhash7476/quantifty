...
| 4b | **`decide()` gates the chain on `futures` alone, so a silently-failed equity ingest still returns `success`.** Exposed live on 2026-07-31: equity was two sessions stale, the options book was dated 2026-07-29, and every automated layer reported success. Data and root cause remediated (`EQUITY_MISS_CACHE_DEFECT.md`), but the pipeline still cannot *report* feed staleness — a four-feed freshness assertion that fails the run is not implemented. | **HIGH** |
