...
**Status:** **RATIFIED (Rev 6) — author-locked, pending Phase-2; NOT yet frozen.** (2026-07-12) The three §3.4 inference decisions are **operator-ratified** (`CSMP_PHASE1_FREEZE_RATIFICATION.md`): D-i CI method = one-sided Student-t by the coverage rule; D-ii = one-sided; D-iii extension design = single-shot. The authors (Claude, DeepSeek) make **no further self-initiated revision**; the one remaining step is the **Phase-2 independent model review by a third frontier model** — neither Claude nor DeepSeek, whose independence is spent. The dossier **FREEZES at Rev 7, after Phase-2 findings are folded** (charter §6 order: review first, then freeze) and before the sealed window (2023-01 → 2026-06) is read (Phase 6).

**Date:** 2026-07-11 (Rev 5 content) · 2026-07-12 (Rev 6 ratification stamp)
...
- **Rev 6 (this)** — **operator ratification applied** (`CSMP_PHASE1_FREEZE_RATIFICATION.md`, 2026-07-12); **author-locked**. Mechanical status change only: **no analytical change from Rev 5** — every number is identical. Rev 7 will carry the immutable **FROZEN** stamp after the Phase-2 review.
...
| Field | Value |
|---|---|
| **Inference — CI method (D-i, §3.4)** | **one-sided 95% Student-t lower bound on the monthly `IC_t` series** — selected by the pre-registered coverage rule (§3.4): at n=42 it covers at **0.957 / one-sided Type-I 0.049** (nominal 0.95 / 0.05). The moving-block **L=12 percentile is retired to a reported robustness arm** (it under-covers: 0.811 / 0.129). **RATIFIED 2026-07-12.** |
...
| Python / duckdb / numpy / scipy versions; store SHA-256; code commit hash | *(pin at build — the only genuinely build-time fields)* |
...
**D-i — CI method (validity). The pre-registered L=12 moving-block percentile CI is invalid at n=42 and is replaced.** A coverage simulation (dev-only; `scripts/csmp/phase1_ci_coverage.py`; population = the empirical 131-month dev IC distribution; iid resampling justified by that series' near-zero serial dependence — the block-L12 CI is only 1.011× the iid CI at n=131) gives, at n=42:

| CI method | 2-sided coverage | 1-sided Type-I | 1-sided power @ dev IC |
|---|---:|---:|---:|
| **Student-t (selected)** | **0.957** | **0.049** | **0.398** |
| iid percentile bootstrap | 0.949 | 0.054 | 0.418 |
| stationary bootstrap (mean-block 3) | 0.924 | 0.064 | 0.453 |
| **moving-block L=12 percentile (incumbent)** | **0.811** | **0.129** | 0.538 |
| *nominal* | *0.950* | *0.050* | *—* |

The incumbent covers at **81%, not 95%** — a one-sided Type-I of **~13%** (≈ 2.6× nominal). Its apparent power (0.538) is **borrowed from that invalidity**: it clears more often than it should under the null *and* the alternative. **Selection rule, pre-registered — pick the method whose n=42 coverage is closest to nominal; guardrail: select on coverage, NOT on narrowness** (this sentence is what stops the choice being relitigated after the seal). The rule selects the **one-sided 95% Student-t lower bound** (coverage 0.957, Type-I 0.049 — essentially exact, and honestly *wider* than L=12's degenerate interval); L=12 is reported as a robustness arm only. Fixing coverage does **not** cost power — it *reveals* the honest power. **Affirmative evidence the rule was calibration and not reverse-engineered for decisiveness (ratification §1.2): it selected *against* power — Student-t's 0.398 is lower than the stationary bootstrap's 0.453 — choosing the better-covered candidate over the more powerful one.**
...
| `mean_IC` lower bound ≤ 0, `mean_IC > 0` | **Inconclusive (underpowered — the modal outcome, ~60%, §3.4)** | **Not Approved. The sealed window is spent once and never re-read** (re-reading nested data is the D-iii multiplicity trap — the naive schedule's FWER is 0.130). Ratified single-shot path (D-iii): (a) build and run the top-40 PAPER consumer as an **explicitly exploratory, non-Approved deployment** — this delivers the "first production Knowledge consumer" §2.2 names as the point, *regardless of verdict*; (b) its **genuinely new forward performance from 2026-07 onward** feeds a **fresh increment-2 pre-registration with its own α** — power bought from new data, not repeated looks at old data. **Charter §6's Approval precondition is an epistemic condition, not a risk gate** — "Approved" labels a *descriptive claim about the sealed window*, not a permission to risk capital, and at PaperBroker scale there is **no capital at risk**. It is **satisfied-in-substance by disclosure**: the consumer is built and run with the artifact disclosed, in code and in every report it emits, as **Not Approved / exploratory** (`CSMP_PHASE1_FREEZE_RATIFICATION.md` §2). **Not licensed:** reporting an Inconclusive result as a confirmation; any LIVE deployment; any parameter change; any re-read of the sealed window. |
...
*End of pre-registration — **RATIFIED (Rev 6), author-locked, NOT yet frozen.** Nothing here reports a result computed on the sealed held-out window (2023-01 → 2026-06); the §2.1 / §3.4 numbers are dev-window (2012–2022) or calendar facts, reproducible from `phase1_prereg_analysis.py`, `phase1_ci_coverage.py`, and `phase1_group_sequential.py` (seed 20260711). D-i, D-ii, and D-iii are operator-ratified (`CSMP_PHASE1_FREEZE_RATIFICATION.md`, 2026-07-12) and the authors make no further self-initiated change. **Remaining before the seal — exactly two items, in order: (1) the Phase-2 independent model review** by a third frontier model that has touched none of this (neither Claude nor DeepSeek — their independence is spent), whose findings are folded in and may reopen any decision here while the window is still untouched; **(2) the Rev 7 FROZEN stamp**, after which the construct fence (universe, score, K=40, metric, baselines, cost model, §5.2 delisting convention, ratified inference/extension design) is immutable and any change is a new pre-registration, not an edit.*
