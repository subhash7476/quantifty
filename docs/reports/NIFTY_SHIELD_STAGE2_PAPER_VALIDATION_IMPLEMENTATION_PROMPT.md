# NiftyShield Stage 2 — PAPER VALIDATED (E008) Implementer Prompt

**For:** DeepSeek V4 (implementer). **Reviewer:** Claude (empirical review only, never implements gate deliverables). **Grantor:** operator / Technical Lead (files Ledger **E008**).
**Strategy:** `nifty_shield_v1` — identity **FROZEN** at `code_ref fe87363`, `config_hash c5b722ff…536c` (Ledger E007, 2026-08-11).
...
Produce a **PAPER Validation Report** (§9.2) proving `nifty_shield_v1` *behaves* — end-to-end through the real composition root against real market data at **zero capital** — across the MM12.5 §4.2 seven evidence items, so the Technical Lead can grant **PAPER VALIDATED (E008)**.
...
1. **Identity is frozen.** Do not edit `strategies/nifty_shield_v1/` in any way that moves `config_hash` or changes the emitted signal stream. The re-cert `code_ref` is `fe87363`; the PAPER window runs at this identity.
...
| **I** | **PAPER Validation Report** (§9.2 permanent record) — window dates, platform commit the window ran on, identity triple (`nifty_shield_v1`, `fe87363`, `c5b722ff…536c`), all seven evidence items assembled, restarts + causes, anomalies + dispositions, regression-suite-green attestation at the run commit. | (documentation) | A (skeleton), B (complete) |

---

## 5. Acceptance gate (what makes E008 grantable — all must hold)
...
- **Operator / Technical Lead** grants **E008** (PAPER VALIDATED) and files the ledger entry after review. The grant records window dates, run commit, identity triple, and the seven evidence items. **Claude does not grant.**
- **E008 does not begin Stage 3.** LIVE CANDIDATE (E009) is a separate package; its MM14 broker-reconciliation / margin-reconciliation infrastructure is built **only** when the candidate reaches Stage 3 — never ahead of need.
