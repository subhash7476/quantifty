...
**Script-generated** — `scripts/psb1/certify_substrate.py`. Code commit `653ade5`.
Real store, read-only. Store stamps: rows **7,158,443**, fenced MAX(trade_date) **2022-12-30**, unfenced MAX **2026-09-11**.
...
| Check | Result | Detail |
|---|:--:|---|
| **Arm A** intra-symbol CA-shape | **HALT** | 83 residue (78 dispositioned, **5** undocumented); 2742 large_genuine |
| **Arm B** cross-symbol handoff | PASS | 0 splice fabrications (0 dispositioned, **0** undocumented) |
| **Arm C** prev_close identity | PASS | 0 violations |
| **Arm D** factor evidence | PASS | 1129 tested, 16 flagged (16 dispositioned, **0** undocumented) |
...
| Structural: row count | **FAIL** | 7,158,443 (expected 7,030,920) |
| Structural: intervals | PASS | 4344 rows, multi-interval symbols: 1 (DTILx2) |
...
83 CA-shaped moves with no matching factor. Dispositioned: 78. **Undocumented (HALT): 5**.
...
| KSE | KSE | 2026-08-17 | -62.2% | magnitude-mismatch | **HALT** |

Large genuine moves (|ret|>=40%, non-CA-shaped, not CA-explained): **2742** — disclosed for operator review, not HALT.

## Arm B — Cross-symbol handoff (shape-free)

0 splice fabrications. All multi-ticker entity handoffs pass (< |20%|).
...
1129 factors with adjacent-session evidence tested. 16 flagged (16 dispositioned, **0** undocumented).
...
| 7 | ISIN-merge splices | B | `7c42a0c` | **YES** | 558 splice fabrications re-appeared, worst: PATANJALI RUCHISOYA->PATANJALI ret=+30923.9% |
| 8 | rename-path splices | B | `d408a68` | **NO** | 0 splices: [] (expected ['CLCIND', 'DELPHIFX', 'INDOSOLAR', 'NEUEON']) |
