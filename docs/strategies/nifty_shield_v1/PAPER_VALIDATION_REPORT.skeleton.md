...
- **Phase B operations:** run `scripts/nifty_shield_paper/session.py` each trading session (per
  ...
  what makes a session count, margin/replay/drill evidence, acceptance gates); publish the 13:00
  fact (the hook), trade the structure, flat by 15:15; capture journal + ledger + telemetry
  (`session.py` finalizes the per-session package automatically, including the load-bearing
  per-session audit); run the mid-window kill-switch drill
