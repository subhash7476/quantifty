...
- **LoopDriver Phase D — SignalSource integration (pull only).** `core/runtime/driver.py`: optional injected `SignalSource`; `on_start` before the loop, `on_bar(bar)` once per bar (after `clock.set_time`, before the bar count, §7.2), `on_stop` on every exit path; the returned `List[SignalEvent]` is collected in list order (never re-ranked) and counted via `signals_pulled`. **Signals are pulled but not routed** — execution (Phase E) deferred until D is green. Source stays optional, so the Phase C inert loop is unchanged when absent; 10 unit tests + `FakeSignalSource`. Full runtime suite: 124 passing. *(docs/DRIVER_SPECIFICATION.md §§5,7.2; docs/LOOPDRIVER_IMPLEMENTATION_PLAN.md Phase D)*

## In Progress

- *(none — next: LoopDriver Phase E execution routing, held by the standing gate "no execution integration until D is green." D is now green, which unblocks E but does not authorize it.)*

## Planned

1. **LoopDriver remaining phases (E–H).** `core/runtime/driver.py`: with the inert loop (A–C) and signal pull (D) complete, wire `ExecutionHandler` routing (E), `RuntimeWatchdog` (F), telemetry (G), and the recovery + reconciliation startup gate (H). The plan's safety-first order lands F + H **before** E so execution is born into a stale-data- and ledger-guarded loop; E must not deploy live until F and H land. Last big runtime pillar; closes Principle 3/5 + §6 operationally. *(docs/DRIVER_SPECIFICATION.md §§4–12, ADR-006; docs/LOOPDRIVER_IMPLEMENTATION_PLAN.md)*
