...
| **Option strategy** | `on_bar` returns option `SignalEvent`s (the strike/expiry selection happens inside the source or via `core/execution/options/selector.py`). | Greeks limits enforced in `handler._check_greek_limits` (`handler.py:706`), not the driver. |
...
**The driver does not handle fills.** `ExecutionHandler` subscribes to broker fills at construction via `self.broker.subscribe_fills(self._handle_broker_fill)` (`handler.py:132`); `_handle_broker_fill` (`handler.py:267`) ingests each `FillEvent` into `order_tracker` / `position_tracker` / `pnl_tracker` and persistence.
...
- **Formula:** `utilisation = (used_current + incremental_est) / cash_balance`; `approved = utilisation <= config.max_capital_utilisation` (default `0.80`; boundary inclusive — equal-to-limit is approved). `used_current = margin_tracker.get_used_margin(self._latest_prices)` (MM9.2-S1: full price cache, not single-symbol); `incremental_est = _estimate_required_margin(qty * effective_multiplier, price) * margin_rate`, where `effective_multiplier` is `canonical_instrument.multiplier` (lot_size for F&O) or `instrument.multiplier` (1.0 for equity) as fallback.
...
- **Denominator (`cash_balance`):** a construction-time scalar set from `ExecutionHandler(initial_capital=...)`, propagated through `build_runner(initial_capital=...)` (MM9.1-S4). Does NOT update with PnL during the session and resets to the configured value on restart (I.H.1, deferred to MM9.2-S4).
...
- **Cold-start residual (spec §8 R2):** At session restart, `_latest_prices` is empty. Positions recovered via `_replay_state` are unpriced by the gate until their first signal warms the cache. Bounded to the first tick cycle per symbol. A one-shot WARNING is emitted at startup when recovered positions exist but the cache is cold (MM9.2-S1 D-S1-4).
