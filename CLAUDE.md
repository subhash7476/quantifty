...
- **`mark_file()` returns 0 when a session has no Category I list, so an unmarked session looks marked.** `data/cas/cas_category.duckdb` ended 2026-08-28, and the 08-31+ carry-forward bars stayed `is_synthetic = FALSE` through a "successful" marker pass. Assert the list is non-empty before marking.
