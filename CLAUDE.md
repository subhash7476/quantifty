...
- **1-min candles**: `data/market_data/nse/candles/1m/{YYYY-MM-DD}.duckdb`
  - Equities (`NSE_EQ|INE...`): **2023-01-02** to present (~196 `NSE_EQ` symbols/session; the earlier "2024-10-17" was wrong — verified across 907 sessions)
  - `NSE_INDEX|Nifty 50`: 2023-01-02 to present
  - `NSE_INDEX|Nifty Bank`: 2023-01-02 to present (backfilled Feb 2026, 292K bars)
  - **PRICES ARE CORPORATE-ACTION ADJUSTED, back-adjusted to the current basis — `equity_bhavcopy` is AS-TRADED.** The two stores disagree by the CA ratio on every session before an ex-date. Verified on 33 bonus/split events across Nifty-100 names: **32 of 32 testable cases adjusted** (the 33rd has no pre-ex 1m data), e.g. RELIANCE 2.00 before its 1:1, KOTAKBANK 5.00 before its 1:5, NMDC 3.00 before its 2:1. See `docs/reports/index_research/PTMS_N100_1M_COVERAGE_BACKFILL_2026-09-13.md` §4/§6
...
- **CAS (Closing Auction Session), live 2026-08-03** — for F&O ("Category I") stocks, continuous cash trading ends **15:15**; the official close is the auction equilibrium struck 15:30–15:35; derivatives trade to **15:40**. In the 1m store this means Category I symbols carry **carry-forward bars** from 15:15 to ~15:27 (`O=H=L=C`, `volume=0`) with the whole auction landing in one print at 15:28/15:29. Those bars are flagged `is_synthetic = TRUE` — **always filter on it**. Session windows come from `core/market/session_schedule.py` (segment-named, date-keyed), never from a bare constant. **Special sessions (Diwali Muhurat) are date-keyed overrides in that module's `SPECIAL_SESSIONS`** — 2023-11-12 18:15–19:15, 2024-11-01 18:00–19:00, 2025-10-21 13:45–14:45, each derived from the store's own bars. A special session **not** in the table falls back to the era schedule and is answered 09:15–15:30, which is wrong in both directions, so **add the next Muhurat before it happens** (standing obligation, like `nse_holidays.py`'s 2027 dates). The auction print **equals** the official bhavcopy close (verified 92–95%; the misses are Category II VWAP closes), so daily-close constructs are unaffected. PIT category: `data/cas/cas_category.duckdb` via `scripts/cas/build_cas_category.py`. Full detail: `docs/reports/CAS_ADAPTATION_REGISTER.md`
...
- **The 1m store and the daily store are on DIFFERENT PRICE BASES.** `candles/1m/` is corporate-action **adjusted** (Upstox historical back-adjusts to the current basis); `equity_bhavcopy` is **as-traded**. Joining them across a CA is wrong by the CA ratio — a factor of 2 for RELIANCE before its 2024-10-28 bonus, 5 for KOTAKBANK before its 2026-01-14 split. Symptoms are silent: an intraday close simply does not match "the close" for that date, and a return computed from a 1m open against a bhavcopy prev_close is garbage on exactly the days that matter. Within the 1m store the series is *continuous* across ex-dates, which is what research wants; the hazard is only at the join. There is no flag recording the basis, so **the check is the ratio test**: `bhavcopy_close / last_traded_1m_close` should be 1.0, and any other value is the CA ratio. Note the same test also picks up benign single-session noise, because NSE's official close is the last-30-minute VWAP, not the last trade.
- **A bar is not a trade.** Since CAS (2026-08-03) roughly 13 minutes per session per F&O symbol are aggregator carry-forward from a stale LTP the feed keeps broadcasting through the halt — real-looking rows with no trades behind them (`O=H=L=C`, `volume=0`). A contiguity gate that counts one bar per slot passes on fabricated data; ISD's did, over 16 certified sessions. **Bar-count completeness is not tradeability** — assert `is_synthetic = FALSE`. And any predicate keyed on `volume=0` is **`NSE_EQ`-only by rule**: indices carry volume 0 on *every* bar, so applying it to `NSE_INDEX` marks the index's own closing value as fabricated. For indices, resolve the era by rule, never by detection.
- **A historical backfill re-introduces every defect the backfill script was written to fix.** Re-ingesting 2026-08-25/26 from the Upstox historical API landed 150,000 fresh rows with `is_synthetic` all FALSE — the exact state the CAS marking pass had just repaired. Re-run the marker after any ingest touching post-CAS sessions, and *never* let the re-run overwrite the copy-first baseline snapshot of an already-marked file.
- **A per-day 1m file's symbol set is whichever universe its writer used, not a universe.** Post-CAS files held 200–231 symbols in blocks that followed ingest runs, so 3–26 F&O names per session (KOTAKBANK and DLF included) were absent while every symbol present mapped cleanly. Check coverage against the universe you need (`scripts/cas/fo_1m_coverage.py`), never against the file. Map F&O names to store keys through `instrument_master`; `symbol_isin` keeps pre-split ISINs that match no bars.
- **`mark_file()` returns 0 when a session has no Category I list, so an unmarked session looks marked.** `data/cas/cas_category.duckdb` ended 2026-08-28, and the 08-31+ carry-forward bars stayed `is_synthetic = FALSE` through a "successful" marker pass. Assert the list is non-empty before marking (`run()` now raises). **Root cause: the table was simply stale** — it is derived from `futures_bhavcopy`, which already ran to 2026-09-11, so a rebuild closed it (2026-09-13: 366 intervals in and out, 210 ends extended, 0 other changes; the 10 sessions then marked, 28,660 bars). **Rebuild it whenever the futures store advances**, or the same silent gap reopens.
