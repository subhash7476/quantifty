"""Skew §4 — beta and sector neutralization.

Reads signals from build_skew.py, computes trailing 252-day beta to Nifty 50,
then residualizes z_skew against beta + sector dummies via cross-sectional OLS
per formation date.
"""
...
def main():
    if not SIG_DB.exists():
        print("ERROR: signals DB not found. Run build_skew.py first.")
        return 1
    ...
    for fi, (fdate,) in enumerate(formations):
        ...
        names = con.execute(f"""
            SELECT underlying, entity, sector, z_skew
            FROM sig.signals
            WHERE formation_date = DATE '{fdate}'
              AND z_skew IS NOT NULL
              AND liquid = TRUE
        """).fetchall()
        ...
        for i, (u, _, _, _, bv) in enumerate(beta_results):
            con.execute(f"""
                UPDATE sig.signals
                SET z_skew_neut = ?, beta = ?
                WHERE formation_date = DATE '{fdate}' AND underlying = '{u}'
            """, [float(neut[i]), bv])

    total = con.execute("SELECT COUNT(*) FROM sig.signals WHERE z_skew_neut IS NOT NULL").fetchone()[0]
    n_f = con.execute("SELECT COUNT(DISTINCT formation_date) FROM sig.signals WHERE z_skew_neut IS NOT NULL").fetchone()[0]
