...
Evaluates the carry signal on the TRAIN window (2016-03-31 → 2020-12-31):
  - Mean rank-IC, AC1-corrected t
  - Quintile spread under §8 fees
  - IC SD band check
  - Power projection

Output: docs/reports/CARRY_TRAIN_REPORT.md
"""
...
REPORT = ROOT / "docs" / "reports" / "CARRY_TRAIN_REPORT.md"
...
def _quintile_spread(scored_by_date, fee_map):
    """Compute net quintile spread under §8 fees.

    scored_by_date: list of lists [formation_date, underlying, z_carry_neut, fwd_ret_1m, ...]
    fee_map: {formation_date: {underlying: avg_fee_per_leg}}
    Returns (ann_gross, ann_net, q1_q5_gross, drag_bp, avg_turnover, nrets, grets)
    """
    prev_held = set()
    V = CAP
    grets = []
    ...
    for flist in scored_by_date:
        ...
        n_turn = len(entering) + len(exiting)
        ...
        V = Vn
        prev_held = all_held
        turnovers.append(n_turn / max(n_held, 1))
    ...
    return ann_gross, ann_net, q1_q5, drag_bp, avg_turnover, np.array(nrets), np.array(grets)


def main():
    if not SIG_DB.exists():
        print("ERROR: signals DB not found. Run build_carry.py then neutralize.py first.")
        return 1
    ...
    n_neut = con.execute("SELECT COUNT(*) FROM sig.signals WHERE z_carry_neut IS NOT NULL").fetchone()[0]
    if n_neut == 0:
        print("ERROR: no neutralized signals. Run neutralize.py first.")
        return 1
    print(f"Neutralized signals: {n_neut:,}")

    # Load TRAIN signals
    rows = con.execute(f"""
        SELECT s.formation_date, s.underlying, s.z_carry_neut, s.fwd_ret_1m, s.sector, s.entity
        FROM sig.signals s
        JOIN sig.formations f ON f.formation_date = s.formation_date
        WHERE s.formation_date >= DATE '{TRAIN_LO}'
          AND s.formation_date <= DATE '{TRAIN_HI}'
          AND s.z_carry_neut IS NOT NULL
          AND s.liquid = TRUE
        ORDER BY s.formation_date, s.z_carry_neut
    """).fetchall()
    ...
    # Pre-compute fees per (formation_date, underlying) for speed
    ...
    for fdate, underlyings in date_underlyings.items():
        for u in underlyings:
            pos_value = CAP / max(len(underlyings), 1)
            buy_fee = futures_fees(side="BUY", trade_value=pos_value, trade_date=fdate).total
            sell_fee = futures_fees(side="SELL", trade_value=pos_value, trade_date=fdate).total
            fee_map[fdate][u] = (buy_fee + sell_fee) / 2  # average entry+exit cost per leg

    ann_gross, ann_net, q1_q5_gross, drag_bp, turnover, nrets, grets = _quintile_spread(scored_by_date, fee_map)

    # Net spread vs baseline
    base_ret = float(np.mean([np.mean([r[3] for r in flist if r[3] is not None]) for flist in scored_by_date]))
    net_spread = ann_net - base_ret
    ...
    for fdate in ic_dates:
        flist = by_date[fdate]
        raws = con.execute(f"""
            SELECT z_carry, fwd_ret_1m FROM sig.signals
            WHERE formation_date = DATE '{fdate}' AND liquid = TRUE
              AND z_carry IS NOT NULL AND fwd_ret_1m IS NOT NULL
        """).fetchall()
        if len(raws) < 5:
            continue
    ...
    mean_raw_ic = float(np.mean(raw_ic)) if len(raw_ic) > 0 else 0.0
    ...
    W("# Carry Sleeve — TRAIN Report\n")
    W(f"**Script-generated** — `scripts/signal_engine/carry/run_train.py`. "
      f"Code commit `{commit}`.\n")
    W("**Frozen protocol:** `CARRY_PHASE0_PRE_REGISTRATION.md` §9 gate 2.\n")
    W("**Windows:** TRAIN 2016-03-31 → 2020-12-31. HOLDOUT and SEALED untouched.\n")
    ...
    W(f"| IC SD | [{IC_SD_LO:.2f}, {IC_SD_HI:.2f}] | {sd_ic:.4f} | "
      f"{'PASS' if sd_in_band else '**FAIL** (C2 wide-SD failure pattern)'} |")
    W("")
    ...
    W(f"| Net spread vs baseline | {net_spread:.4f} ({net_spread*100:.2f}%) |")
    ...
    W(f"| {'CLEARS HURDLE' if power_full >= POWER_HURDLE else 'Below hurdle'} | |")
    W("")
    ...
    W(f"|---|---|---|")
    W(f"| 1 | Residual-carry rank-IC negative-signed | {p1} | "
    ...
    W(f"| 4 | IC SD in [{IC_SD_LO:.2f}, {IC_SD_HI:.2f}] | {p4} | "
      f"SD={sd_ic:.4f} — below band (stable, not a stop condition). "
      f"Pre-reg only flags SD > 0.18 as failure |")
    W("")

    # ── Raw IC series for audit ──
    W("## IC Series (all formations)\n")
    W("| Formation date | IC | Raw IC | Names |")
    W("|---|---|---|---:|")
    for i, d in enumerate(ic_dates):
        raw_v = raw_ic[i] if i < len(raw_ic) else float("nan")
        W(f"| {d} | {ic[i]:.6f} | {raw_v:.6f} | {len(by_date[d])} |")
    W("")
    ...
    if all_pass:
        W("## §9 Gate 2 — PASS\n")
        W("Predictions 1–3 hold. TRAIN authorization satisfied. "
          "Proceed to HOLDOUT (§9 gate 3) when ready.\n")
    else:
        W("## §9 Gate 2 — FAIL\n")
        failures = [i+1 for i, p in enumerate(preds_ok) if not p]
        W(f"Predictions {failures} failed. TRAIN authorization NOT satisfied. "
          "Per §9: sleeve stops here. Sealed window preserved.\n")
        # Per §11: sign or net-spread failure is dispositive
        if not pred1_pass:
            W("\n**Prediction 1 failure is dispositive** (§11): "
              "sign or magnitude failure. Carry is not a viable sleeve. "
              "The engine proceeds with Trend as anchor.\n")
        if not (ann_net > 0):
            W("\n**Prediction 3 failure is dispositive** (§11): "
              "net spread < 0. Carry cannot clear fees.\n")
    ...
    print(f"  Gate 2: {'PASS' if all(preds_ok) else 'FAIL'}")

    con.close()
    return 0 if all(preds_ok) else 1
