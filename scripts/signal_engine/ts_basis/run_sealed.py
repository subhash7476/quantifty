"""TS Basis — SEALED read (one-shot, unrepeatable).

Protocol: TS_BASIS_SEALED_READ_PROTOCOL.md §1-§4 (frozen)
Pre-reg:  TS_BASIS_PHASE0_PRE_REGISTRATION.md (frozen, SHA 07265b50...)
Construction: z_ts = (basis - trailing_mean) / trailing_std

One run only. Snapshot-and-log before interpreting.
...
REPORT = ROOT / "docs" / "reports" / "TS_BASIS_SEALED_REPORT.md"
SNAPSHOT = ROOT / "docs" / "reports" / "TS_BASIS_SEALED_SNAPSHOT.json"
...
def main():
    ...
    snapshot = {
        ...
        "frozen_artifacts": {
            "TS_BASIS_PHASE0_PRE_REGISTRATION.md": "07265b507179667588d06cb35c1e98c72bd065a3bbf95cf9a6c7d8b996a1ad84",
            "TS_BASIS_SEALED_READ_PROTOCOL.md": "8bdec782af02572bce362b3a8ca0fb9a486648be827e19eeaef2558a7dbbfcd5",
            "governance/rfa/declarations/ts_basis.py": "pending",
        },
    }
    ...
    for fd in formation_dates:
        ...
        for side_positions, reb in [(long_positions, rl), (short_positions, rs)]:
            for u in set(side_positions) | set(reb):
                ...
                side = "BUY" if delta > 0 else "SELL" if side_positions is long_positions else "SELL" if delta > 0 else "BUY"
    ...
    months = len(g_arr)
    ann_gross = float(np.prod(1 + g_arr) ** (12.0 / months) - 1) if months > 0 else 0.0
    ann_net = float(np.prod(1 + n_arr) ** (12.0 / months) - 1) if months > 0 else 0.0
    ...
    lines = []
    a = lines.append
    a("# TS Basis — SEALED Read Report\n")
    a(f"**One-shot, script-generated** — `scripts/signal_engine/ts_basis/run_sealed.py`. Code commit `{commit}`.\n")
    a(f"**Run timestamp:** {run_ts}\n")
    a(f"**Protocol:** `TS_BASIS_SEALED_READ_PROTOCOL.md` (frozen, SHA `8bdec782...`).\n")
    a(f"**Pre-registration:** `TS_BASIS_PHASE0_PRE_REGISTRATION.md` (frozen, SHA `07265b50...`).\n")
    ...
    a(f"| Return periods | {months} |"); a(f"| Net > 0 | {'**PASS**' if net_pass else '**FAIL**'} |\n")
    ...
    if gate:
        a("**SEALED VERDICT: PASS** — TS Basis is a validated alpha across discovery "
          "(TRAIN) + two out-of-sample windows (HOLDOUT, SEALED). The research phase "
          "closes. Work moves to implementation.\n")
    else:
    ...
    snapshot["results"] = {
        ...
        "portfolio": {"ann_gross": ann_gross, "ann_net": ann_net, "fee_drag_bp": drag, "total_fees": total_fees, "total_slippage": total_slippage, "avg_turnover": avg_to, "return_periods": months, "fee_breakdown": dict(fb)},
