"""TS Basis — Drawdown / Regime Profile.

Runs the simulation over TRAIN+HOLDOUT and reports worst month, max DD,
Sharpe, and monthly return distribution.

Bridge: TS_BASIS_PHASE0_PRE_REGISTRATION.md §6 (falsifiable predictions).
Output: docs/reports/TS_BASIS_DRAWDOWN_REPORT.md
"""
...
REPORT = ROOT / "docs" / "reports" / "TS_BASIS_DRAWDOWN_REPORT.md"
...
def main():
    ...
    lines = []
    a = lines.append
    a("# TS Basis — Drawdown / Regime Profile\n")
    ...
    for label, (lo, hi) in WINDOWS.items():
        ...
        sharpe = float(np.mean(nets) / np.std(nets, ddof=1) * np.sqrt(12)) if len(nets) > 1 and np.std(nets, ddof=1) > 0 else 0
        pos_pct = float(np.mean(nets > 0)) * 100

        a(f"## {label} ({len(nets)} months)\n")
        a(f"| Metric | Value |")
        a(f"|---|--:|")
        a(f"| Annualized net | {r['ann_net']*100:+.2f}% |")
        a(f"| Worst month | {worst*100:+.2f}% |")
        a(f"| Best month | {best*100:+.2f}% |")
        a(f"| Max drawdown | {max_dd*100:+.2f}% |")
        a(f"| Monthly std | {np.std(nets, ddof=1)*100:.2f}% |")
