"""TS Basis Daily — HOLDOUT Read.

Mirror of TS Basis run_holdout.py for daily cadence.

Output: docs/reports/TS_BASIS_DAILY_HOLDOUT_REPORT.md
"""
...
REPORT = ROOT / "docs" / "reports" / "TS_BASIS_DAILY_HOLDOUT_REPORT.md"
...
def main():
    ...
    lines = []
    a = lines.append
    a("# TS Basis Daily — HOLDOUT Read Report\n")
    a(f"**Script-generated** — `scripts/signal_engine/ts_basis_daily/run_holdout.py`. Code commit `{commit}`.\n")
    ...
    a(f"**Evidence floor:** Bonferroni α = {ALPHA} (m >= 2). One-sided test.\n")
    a("")
    ...
    if gate_pass:
        a("**HOLDOUT VERDICT: PASS** — TS Basis Daily clears both pre-registered acceptance criteria.\n")
    elif falsified:
        a("**HOLDOUT VERDICT: FAIL (FALSIFIED)** — Both IC and net fail.\n")
    else:
        a("**HOLDOUT VERDICT: INCONCLUSIVE** — One criterion passed, one failed.\n")
