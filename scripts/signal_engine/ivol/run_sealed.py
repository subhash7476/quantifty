"""IVOL SEALED read — one-shot, unrepeatable.

Protocol:  IVOL_SEALED_READ_PROTOCOL.md §1-§5
Pre-reg:   IVOL_PHASE0_PRE_REGISTRATION.md (sign=-1, negative)
Construction: IVOL_PHASE0_PRE_REGISTRATION.md §3-§4

One run only. Snapshot-and-log before interpreting. Output: IVOL_SEALED_REPORT.md
Book: long bottom quintile (low z_ivol = low vol), short top quintile (high vol).
"""
...
REPORT = ROOT / "docs" / "reports" / "IVOL_SEALED_REPORT.md"
SNAPSHOT = ROOT / "docs" / "reports" / "IVOL_SEALED_SNAPSHOT.json"

SEALED_LO = date(2023, 1, 1)
SEALED_HI = date(2026, 7, 20)
POWER_N_STAR = 42

# Fee model with SEALED-era STT tiers (identical to Carry's run_sealed.py)
...
def _load_adva(con, formation_date: date, underlyings: list) -> dict:
    if not underlyings:
        return {}
...
def _compute_targets(rows, adva, n_quintile):
    """IVOL equal-weight quintile: LONG bottom (low z_ivol = low vol), SHORT top (high vol)."""
    sorted_rows = sorted(rows, key=lambda r: r[1])
    long_set = {r[0] for r in sorted_rows[:n_quintile]}     # bottom z -> low vol -> LONG
    short_set = {r[0] for r in sorted_rows[-n_quintile:]}   # top z -> high vol -> SHORT
...
def main():
    ...
    snapshot = {
        ...
        "sign": "-1",
        "portfolio": "quintile_equal_weight_long_lowvol",
        "fee_model": {
            "stt_tiers": {
                "pre_2023_04": 0.00010,
        ...
        "frozen_artifacts": {
            "IVOL_PHASE0_PRE_REGISTRATION.md":
                "82ed96f9c493852c81c7452f54dee20cd452d7656ebb243cfe5fb406cd1c3f4d",
            "IVOL_SEALED_READ_PROTOCOL.md":
                "d54e955b9134c3287715ba255d58db1fb94c06c3e3fb0447b2f706ac8d627201",
            "governance/rfa/declarations/ivol.py":
                "d7ebcbcc743dc708ab6bb0896bf5bde8187d72f38d6b62fe58a24e203e48989f",
        },
    }
    ...
    # NEGATIVE one-sided p: P(T <= tstat) under H0
    p_one = float(student_t.cdf(tstat, n_ic - 1)) if n_ic > 1 else 1.0
    ...
    sign_correct = mean_ic < 0  # IVOL: negative
    ic_pass = sign_correct and p_one < ALPHA

    print(f"  IC: mean={mean_ic:+.4f}  sd={sd_ic:.4f}  t={tstat:.4f}  p(neg)={p_one:.6e}  AC1={ac1:.4f}")
    print(f"  Sign correct (negative): {sign_correct}  IC gate: {'PASS' if ic_pass else 'FAIL'}")
    ...
    a("# IVOL Sleeve — SEALED Read Report\n")
    a(f"**One-shot, script-generated** — `scripts/signal_engine/ivol/run_sealed.py`. "
      f"Code commit `{commit}`.\n")
    a(f"**Run timestamp:** {run_ts}\n")
    a("**Protocol:** `IVOL_SEALED_READ_PROTOCOL.md` §1–§5 (frozen, SHA-256 "
      "`d54e955b9134c3287715ba255d58db1fb94c06c3e3fb0447b2f706ac8d627201`).\n")
    a("**Pre-registration:** `IVOL_PHASE0_PRE_REGISTRATION.md` (frozen, SHA-256 "
      "`82ed96f9c493852c81c7452f54dee20cd452d7656ebb243cfe5fb406cd1c3f4d`).\n")
    a("**Construction:** `IVOL_PHASE0_PRE_REGISTRATION.md` §3–§4 (frozen).\n")
    a(f"**Window:** SEALED {SEALED_LO} → {SEALED_HI} ({n_formations} formations, "
      f"{n_ic} with IC).\n")
    a(f"**Sign:** −1 (long low idiosyncratic vol, short high).\n")
    a("")

    a("---\n")
    a("## 1. Rank-IC (Negative-Sign, One-Sided)\n")
    a(f"| Metric | Value |")
    a(f"|---|---|")
    ...
    a(f"| p-value (one-sided, negative direction) | {p_one:.6e} |")
    a(f"| AC1 | {ac1:.4f} |")
    if nw_t is not None:
    ...
    a(f"|---|---|---|")
    a(f"| Negative-sign IC significant (one-sided, alpha={ALPHA}) | "
    ...
    if gate_pass:
        a("**SEALED VERDICT: PASS** — IVOL is a validated alpha across discovery "
          "(TRAIN) + two out-of-sample windows (HOLDOUT, SEALED).\n")
        a("Combined with Carry (SEALED-validated) and the gate-4 composite clearance "
          "(power 0.9854, signal correlation −0.04), the **2-sleeve engine is "
          "research-complete.** Work moves to implementation design (sizing via "
          "`NseMarginEngine`, execution, risk limits).\n")
    else:
        a("**SEALED VERDICT: FAIL** — the effect did not survive the true holdout.\n")
        a("IVOL is dead; the sealed window is spent. An honest terminal result, "
          "reported as-is.\n")
