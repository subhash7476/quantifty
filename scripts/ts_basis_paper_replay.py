"""TS Basis shadow sleeve — historical PAPER replay.

Reuses CarryRebalancerHook unchanged: points at ts_facts.duckdb
(which stores z_ts in the z_carry_neut column for compatibility).

Runs TRAIN + HOLDOUT replay via DailyBhavcopyProvider + LoopDriver.
...
_logger = logging.getLogger("ts_basis_replay")
...
def _compute_returns(captured):
    ...
    months = len(net)
    ann = float((np.prod([1.0 + r for r in net]) ** (12.0 / months) - 1)) if months > 0 else 0.0
    return net, ann
...
def main():
    ...
    if not TS_FACTS_DB.exists():
        _logger.error("TS Basis facts not found: %s", TS_FACTS_DB)
        _logger.error("Run scripts/signal_engine/ts_basis/publish_facts.py first.")
        return 1
    ...
    with CarryMetricsDB(str(PROD_DB)) as db:
        for label, (lo, hi) in WINDOWS.items():
            ...
            run_id = f"ts-basis-replay-{label.lower()}-{now_ts[:10]}"
            db.write_run_metadata(
                run_id=run_id, git_commit=commit,
                generated_at=datetime.utcnow(),
                window_label=f"TS_{label}",
