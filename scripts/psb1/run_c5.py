"""PSB-1 Phase 2 C5 battery: script-generated report (low-vol anomaly, monthly, banded)."""
...
REPORT = ROOT / "docs" / "reports" / "PSB1_C5_REPORT.md"
...
def main():
    ...
    print("Loading panel for C5...")
    panel = load_panel(db_path=STORE, cutoff=DEV_HI)

    print("Making C5 score function (low-vol anomaly, banded)...")
    fn = make_score_fn(panel, "C5", reg_pos=reg_pos)

    print("Evaluating C5 candidate...")
    result = evaluate_candidate(panel, "C5", fn, db_path=STORE, reg_pos=reg_pos)
    ...
    w = []
    W = w.append
    W("# PSB-1 Phase 2 C5 Battery Report — Low-Vol Anomaly (Monthly, Banded)")
    W("")
    W("**Script-generated** — `scripts/psb1/run_c5.py`. Deterministic run (§10).")
    W("")
    W("| Field | Value |")
    W("|---|---|")
    ...
    W("")
    W("Score: s = −σ_i (252-day daily vol). Low vol → high score. Banded holding")
    W("(C5_EXIT_BAND=0.40): entities within top 40% of prior period are retained.")
    W("")
    W("## §6 Metrics")
    W("")
    W("| Metric | Value |")
    ...
    print(f"C5 summary: n={r.n_dates}, mean IC={r.mean_ic:.6f}, t={r.tstat:.4f}, p={r.pvalue:.6e}")
