"""PSB-1 Phase 2 C4 battery: script-generated report (delivery-conditioned reversal)."""
...
REPORT = ROOT / "docs" / "reports" / "PSB1_C4_REPORT.md"
...
def main():
    ...
    print("Loading panel for C4...")
    panel = load_panel(db_path=STORE, cutoff=DEV_HI)

    print("Making C4 score function (C1 x C3 interaction)...")
    fn = make_score_fn(panel, "C4", reg_pos=reg_pos)

    print("Evaluating C4 candidate...")
    result = evaluate_candidate(panel, "C4", fn, db_path=STORE, reg_pos=reg_pos)
    ...
    w = []
    W = w.append
    W("# PSB-1 Phase 2 C4 Battery Report — Delivery-Conditioned Reversal")
    W("")
    W("**Script-generated** — `scripts/psb1/run_c4.py`. Deterministic run (§10).")
    W("")
    W("| Field | Value |")
    W("|---|---|")
    ...
    W("")
    W("Score: s = c1 · (1 − 2·p(c3)) — C1 reversal weighted by delivery percentile.")
    W("High-delivery (p→1) flips the reversal sign; low-delivery (p→0) passes reversal through.")
    W("")
    W("## §6 Metrics")
    W("")
    W("| Metric | Value |")
    ...
    print(f"C4 summary: n={r.n_dates}, mean IC={r.mean_ic:.6f}, t={r.tstat:.4f}, p={r.pvalue:.6e}")
