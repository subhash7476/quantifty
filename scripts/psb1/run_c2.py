"""PSB-1 Phase 2 C2 battery: script-generated report (residual reversal)."""
...
REPORT = ROOT / "docs" / "reports" / "PSB1_C2_REPORT.md"
...
def main():
    ...
    print("Loading panel for C2...")
    panel = load_panel(db_path=STORE, cutoff=DEV_HI)

    print("Making C2 score function (precomputes market returns)...")
    fn = make_score_fn(panel, "C2", reg_pos=reg_pos)

    print("Evaluating C2 candidate...")
    result = evaluate_candidate(panel, "C2", fn, db_path=STORE, reg_pos=reg_pos)
    ...
    w = []
    W = w.append
    W("# PSB-1 Phase 2 C2 Battery Report — Residual Reversal")
    W("")
    W("**Script-generated** — `scripts/psb1/run_c2.py`. Deterministic run (§10).")
    W("")
    W("| Field | Value |")
    W("|---|---|")
    ...
    print(f"C2 summary: n={r.n_dates}, mean IC={r.mean_ic:.6f}, t={r.tstat:.4f}, p={r.pvalue:.6e}")
