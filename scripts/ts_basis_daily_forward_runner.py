"""TS Basis Daily — forward PAPER runner.

Mirror of ts_basis_forward_runner.py for daily cadence.
Runs LoopDriver + DailyBhavcopyProvider forward from today.

Usage: python scripts/ts_basis_daily_forward_runner.py [--dry-run]
"""
...
_logger = logging.getLogger("ts_basis_daily_forward")
...
def main():
    ...
    run_id = f"ts-basis-daily-forward-{now_ts[:10]}"
    ...
    _logger.info("TS Basis Daily forward: %d symbols, run=%s, start=%s", len(symbols), run_id, start_date)
    ...
    _logger.info("TS Basis Daily forward PAPER started. Run ID: %s", run_id)
    ...
    db.close()
    _logger.info("TS Basis Daily forward PAPER stopped. Hash: %s", dhash)
    return 0
