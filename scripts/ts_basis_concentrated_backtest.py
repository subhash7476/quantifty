"""TS Basis — concentrated backtest (top-5 longs, top-5 shorts).

Reuses CarryRebalancerHook with max_positions_per_leg=5.
Trims each formation to the top 5 by z_ts (highest → LONG)
and bottom 5 (lowest → SHORT), then renormalizes within legs.
Runs TRAIN + HOLDOUT via LoopDriver REPLAY.
"""
...
_logger = logging.getLogger("ts_basis_b5")

TS_FACTS = ROOT / "data" / "signal_engine" / "ts_basis" / "ts_facts.duckdb"
TS_SIG = ROOT / "data" / "signal_engine" / "ts_basis" / "ts_signals.duckdb"
...
def _max_dd(monthly):
    eq = np.cumprod(1.0 + np.array(monthly))
...
def _sharpe(monthly):
    ...
    if len(arr) < 2 or np.std(arr, ddof=1) == 0:
        return 0.0
    return float(np.mean(arr) / np.std(arr, ddof=1) * np.sqrt(52))
...
def _compute_returns(captured):
    ...
    months = len(net)
    ann_net = float((np.prod([1.0 + r for r in net]) ** (52.0 / months) - 1)) if months > 0 else 0.0
    ann_gross = float((np.prod([1.0 + r for r in gross_list]) ** (52.0 / months) - 1)) if months > 0 else 0.0
...
def main():
    ...
    print(f"TS Basis Concentrated (top-{MAX_POSITIONS}/bottom-{MAX_POSITIONS})")
    print(f"Gross: Rs {GROSS_EXPOSURE/1e7:.1f} Cr, Qty: 5 bp/side, Weekly\n")
