...
core/execution/futures/futures_fees.py docstring):
  ...
  STT (futures, SELL only):    nil pre-2008 -> 0.017% (2008-06-01) -> 0.01%
                               (2013-06-01, Budget 2013) -> 0.0125%
                               (2023-04-01, Finance Act 2023) -> 0.02%
                               (2024-10-01). Zero on buy.
  NSE F&O txn (both legs):     0.0021% -> 0.00189% (2024-10-01, SEBI MII
                               rationalization).
  SEBI turnover:               0.0001% (Rs 10/crore), both legs.
  Stamp duty (BUY only):       0.01% pre-2020-07-01 (Maharashtra-representative
                               assumption) -> 0.002% central rate after.
...
class TestRateSchedules:
    def test_stt_pre_finance_act_2023(self):
        assert stt_futures_rate(date(2013, 6, 1)) == 0.0001
        assert stt_futures_rate(date(2019, 10, 1)) == 0.0001
        assert stt_futures_rate(date(2023, 3, 31)) == 0.0001

    def test_stt_2008_tier_and_pre_2008_nil(self):
        # Derivatives were brought into STT in 2008; there was no rate before.
        assert stt_futures_rate(date(2008, 6, 1)) == 0.00017
        assert stt_futures_rate(date(2007, 12, 31)) == 0.0

    def test_stt_finance_act_2023_to_2024(self):
        assert stt_futures_rate(date(2023, 4, 1)) == 0.000125
    ...
    def test_exchange_txn_post_oct_2024(self):
        assert exchange_txn_rate(date(2024, 10, 1)) == 0.0000189
        assert exchange_txn_rate(date(2026, 1, 1)) == 0.0000189

    def test_stamp_duty_both_eras(self):
        # Pre-2020-07-01 is the Maharashtra-representative 0.01%; the uniform
        # central derivatives rate of 0.002% starts 2020-07-01.
        assert stamp_duty_rate(date(2013, 6, 1)) == 0.0001
...
class TestRoundTrip:
    def test_post_oct_2024_hand_computed(self):
        ...
        assert f.exchange_txn == pytest.approx(2 * v * 0.0000189)
        ...
        assert f.gst == pytest.approx(0.18 * gst_base)
    ...
    def test_stt_stamp_outside_gst_base(self):
        ...
        f = futures_round_trip_fees(entry_value=v, exit_value=v,
                                    entry_date=d)
        assert f.gst == pytest.approx(0.18 * (f.brokerage + f.exchange_txn
                                              + f.sebi_fee))
    ...
    def test_2013_era_service_tax(self):
        ...
        assert f.gst == pytest.approx(0.1236 * (f.brokerage + f.exchange_txn
                                                + f.sebi_fee))
        assert f.stamp_duty == pytest.approx(1000.0)    # 0.01% buy leg, pre-2020
